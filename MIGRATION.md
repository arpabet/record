...
| `raftgrpc` | **none** | ⚠️ no arpabet repo; folded into `go.arpabet.com/sprint/raftmod` — see below |
...
### 2. raftgrpc
`main.go` uses `raftgrpc.RaftCommand()` and `raftgrpc.RaftGrpcServer()`. There is
no `arpabet/raftgrpc`; this functionality now lives in
`go.arpabet.com/sprint/raftmod`. Locate the equivalent symbols there and drop the
codeallergy `raftgrpc` require.
